var d = Object.defineProperty,
    ...
    l = (e, t) => {
        for (var n in t || (t = {})) f.call(t, n) && u(e, n, t[n]);
        if (h)
            for (var n of h(t)) m.call(t, n) && u(e, n, t[n]);
        return e
    };
...
const getKeyCache = (e, t) => e + t,
    ...
    serviceWorkerCacheData = e => {
        ...
        } = e, a = new Response(JSON.stringify(""));
        return "POST" === r ? e.respondWith(t.json().then((e => caches.open(n).then((t => t.put(n, new Response(JSON.stringify(e))))))).then((() => a))) : e.respondWith(caches.open(n).then((e => e.match(n).then((e => e || a)) || a)))
    },
    ...
    CDA_REQUEST_PATHNAME = "/cdaredirect";
...
const HOME_PAGE = "/",
    ...
    proxyPost = (e, t, n = !0) => {
        const r = new URL(e),
            a = t.clone(),
            i = l({}, CUSTOM_REFERRER_PATHNAME.has(r.pathname) && {
                "X-Custom-Referer": t.referrer
            });
        for (const [o, s] of a.headers.entries()) i[o] = s;
        return a.blob().then((r => {
            const a = t,
                o = new Request(e, l({
                    body: r,
                    headers: i,
                    ...
                    isHistoryNavigation: a.isHistoryNavigation
                }));
            return fetch(o)
        }))
    },
    ...
    proxyFetch = e => {
        const {
            rewrites: t,
            actualUrl: n
        } = config, r = new URL(e.request.url), a = t[r.pathname] || r.pathname, i = n + a + r.search;
        if (CDA_REQUEST_PATHNAME === r.pathname) return proxyPost(i, e.request, !1);
        if ("navigate" === e.request.mode) {
            const e = t[HOME_PAGE] || r.pathname,
                a = n + e + r.search;
            return navigationFetch(a)
        }
        return ["POST", "PUT"].includes(e.request.method) ? proxyPost(i, e.request) : fetch(i, e.request)
    };
...
self.addEventListener("fetch", (e => {
    const t = new URL(e.request.url);
    return /^\/sw-cache\/.+/.test(t.pathname) ? serviceWorkerCacheData(e) : FORBIDDEN_ACTUAL_PATHNAMES.has(t.pathname) ? fetch(e.request) : "/get-is-domain-available" === t.pathname ? e.respondWith(getIsDomainBlockedPromise(e).then((e => new Response(e ? "false" : "true"))).catch((() => new Response("false")))) : "/turn-on-force-actual-domains-usage" === t.pathname ? (config.isForceActualDomainsLogicUsage = !0, e.respondWith(new Response("ok"))) : config.isForceActualDomainsLogicUsage && APPLY_ACTUAL_DOMAIN_LOGIC_URLS_ARR.has(t.origin) && !isMxDomain ? e.respondWith(fetchFromActualDomain(e).catch((() => fetchFromCache(config.version, NOT_FOUND_PAGE)))) : APPLY_ACTUAL_DOMAIN_LOGIC_URLS_ARR.has(t.origin) && !isMxDomain ? e.respondWith(fetchFromTargetDomainOrActualDomain(e)) : void 0
